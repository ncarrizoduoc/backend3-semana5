package com.duoc.BffBancoMovil.exception;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler{
    ...
    public ResponseEntity<ErrorResponse> handleAll(Exception ex, WebRequest request){
        ...
        ErrorResponse body = new ErrorResponse(LocalDateTime.now(), HttpStatus.INTERNAL_SERVER_ERROR.value(),
            "Internal Server Error", extractMessage(ex), request.getDescription(false));
    ...
    private String extractMessage(Exception ex) {
        if (ex instanceof RestClientResponseException restClientException) {
            try {
                ...
                String message = objectMapper.readTree(restClientException.getResponseBodyAsString())
                    .path("message")
                    .asText(null);
                if (message != null) {
                    return message;
                ...
                logger.debug("Could not parse the microservice error response", parseException);
            }
        }
